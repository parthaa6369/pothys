...
FROM base AS build
WORKDIR /app/microservices/analytics-service
...
COPY --from=build /app/microservices/analytics-service/dist ./dist
COPY --from=build /app/microservices/analytics-service/package.json ./microservices/analytics-service/
