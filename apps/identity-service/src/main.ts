...
/**
 * Identity Service Microservice Bootstrap (Hot Reload Test)
 *
...
// Bootstrap the User Management microservice using the core bootstrap function
startApplication({
  appModule: AppModule,
  ...
  swagger: {
    ...
    setupFunction: (app) =>
      setupSwagger(app, {
        description: "API documentation for the User Managements microservice",
        version: "1.0",
        path: "api-docs",
      }),
  },
  ...
  .then((data) => {
    Logger.log(`📚 Swagger Docs : http://[::1]:${process.env.PORT}/api-docs`);
  })
  .catch((error) => {
