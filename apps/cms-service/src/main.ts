...
/**
 * Compliance Management Microservice Bootstrap
 *
...
// Bootstrap the Compliance Management microservice using the core bootstrap function
startApplication({
  appModule: AppModule,
  port: process.env.PORT ? parseInt(process.env.PORT) : 3000,
  globalPrefix: "api",
  corsOrigin: ["http://localhost:5173", "http://localhost:3000", "*"], // Allow frontend ports
  ...
  swagger: {
    ...
    setupFunction: (app) =>
      setupSwagger(app, {
        description:
          "API documentation for the Compliance Managements microservice",
        version: "1.0",
        path: "api-docs",
      }),
  },
  ...
  console.error("❌ Failed to start Compliance Management service:", error);
  process.exit(1);
});
