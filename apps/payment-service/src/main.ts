...
/**
 * File Management Microservice Bootstrap
 *
...
// Bootstrap the File Management microservice using the core bootstrap function
startApplication({
  appModule: AppModule,
  ...
  swagger: {
    ...
    setupFunction: (app) =>
      setupSwagger(app, {
        description: "API documentation for the File Management microservice",
        version: "1.0",
        path: "api-docs",
      }),
  },
  ...
  console.error("❌ Failed to start File Management microservice:", error);
  process.exit(1);
});
