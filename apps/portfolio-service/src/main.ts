...
/**
 * Diffusion Management Microservice Bootstrap
 *
...
// Bootstrap the Diffusion Management microservice using the core bootstrap function
startApplication({
  appModule: AppModule,
  ...
  validation: {
    ...
    whitelist: true,
    forbidNonWhitelisted: false, // Allow extra properties in DTOs
  },
  ...
  swagger: {
    ...
    setupFunction: (app) =>
      setupSwagger(app, {
        description:
          "API documentation for the Diffusion Managements microservice",
        version: "1.0",
        path: "api-docs",
      }),
  },
  ...
  console.error("❌ Failed to start Diffusion Management service:", error);
  process.exit(1);
});
